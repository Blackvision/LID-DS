...
class ScorePlotBoth:
    ...
    def __init__(self, scenario_path):
        ...
        self._first_window_of_exploit_recording_index_list = []
        self._first_window_of_normal_recording_index_list = []
        self._first_window_after_exploit_index_list = []
        self._exploit_time = None
        self._first_window_after_exploit = False
        self.threshold = 0.0
        self._first_window_of_cfp_list_exploit = []
        self._last_window_of_cfp_list_exploit = []
        self._first_window_of_cfp_list_normal = []
        self._last_window_of_cfp_list_normal = []
    ...
    def add_to_plot_data(self, score: float, time_window_start, cfa_indices: tuple):
        ...
        self._first_window_of_cfp_list_exploit = cfa_indices[0]
        self._last_window_of_cfp_list_exploit = cfa_indices[1]
        self._first_window_of_cfp_list_normal = cfa_indices[2]
        self._last_window_of_cfp_list_normal = cfa_indices[3]
    ...
    def feed_figure(self):
        ...
        if len(self._first_window_of_cfp_list_normal) > 1 and len(self._last_window_of_cfp_list_normal) > 1:
            for i, j in zip(self._first_window_of_cfp_list_normal, self._last_window_of_cfp_list_normal):
        ...
        while not done:
            exploit_window_start = self._first_window_after_exploit_index_list[exploit_start_index]
            for i in range(recording_start_index, len(self._first_window_of_exploit_recording_index_list)):
                if self._first_window_of_exploit_recording_index_list[i] > exploit_window_start:
                    exploit_window_end = self._first_window_of_exploit_recording_index_list[i]
                    recording_start_index = i
                    break
            ax2.axvspan(exploit_window_start, exploit_window_end, color="lightcoral")
            exploit_start_index += 1
            if exploit_start_index == len(self._first_window_after_exploit_index_list):
                done = True

        # cfp windows for exploit subplot
        for i, j in zip(self._first_window_of_cfp_list_exploit, self._last_window_of_cfp_list_exploit):
