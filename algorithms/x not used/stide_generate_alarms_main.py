    ...
    for select_scenario_number in range(0, len(scenario_names)):
        for thread_aware in [False, True]:
            for ngram_length in [3, 5, 7]:
                ...
                results['scenario'] = lid_ds_version[select_lid_ds_version_number] + "/" + scenario_names[
                    select_scenario_number]
                ...
                if generate_and_write_alarms:
                    with open(
                            f"results/alarms_{config_name}_{lid_ds_version[select_lid_ds_version_number]}_{scenario_names[select_scenario_number]}.json",
                            'w') as jsonfile:
