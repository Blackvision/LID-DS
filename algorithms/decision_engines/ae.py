...
class AE(BuildingBlock):
    ...
    """
    # 600; 28800; 172800
    def __init__(self, input_vector: BuildingBlock, mode: AEMode = AEMode.LOSS, batch_size=256, max_training_time=172800,
    ...
    def fit(self):
        ...
        with tqdm(total=self._max_training_time, unit=" epoch", bar_format="{l_bar}{bar}| {n:0.1f}/{total}s") as bar:
            last_ts = time.time()
            epoch_counter = 0
            bar.set_description(f"fit AE: {epoch_counter}|{0}/{self._early_stopping_num_epochs}|None".rjust(27),
                                refresh=True)
            while True:
                epoch_counter += 1
