...
class ScorePlot:
    ...
    def __init__(self, scenario_path):
        ...
        self._first_datapacket_of_exploit_recording_index_list = []
        self._first_datapacket_of_normal_recording_index_list = []
        self._first_datapacket_after_exploit_index_list = []
        self._exploit_time = None
        self._first_datapacket_after_exploit = False
        self.threshold = 0.0
        self._first_datapacket_of_cfp_list_exploit = []
        self._last_datapacket_of_cfp_list_exploit = []
    ...
    def add_to_plot_data(self, score: float, datapacket: Datapacket, cfa_indices: tuple):
        """
        ...
        if self._exploit_time is not None:
            ...
            if datapacket_time >= self._exploit_time and self._first_datapacket_after_exploit is False:
                self._first_datapacket_after_exploit_index_list.append(len(self._anomaly_scores_exploits))
                self._first_datapacket_after_exploit = True

        if self._exploit_time is None:
            self._anomaly_scores_no_exploits.append(score)

        self._first_datapacket_of_cfp_list_exploit = cfa_indices[0]
        self._last_datapacket_of_cfp_list_exploit = cfa_indices[1]
        self._first_datapacket_of_cfp_list_normal = cfa_indices[2]
        self._last_datapacket_of_cfp_list_normal = cfa_indices[3]
    ...
    def feed_figure(self):
        ...
        while not done:
            exploit_window_start = self._first_datapacket_after_exploit_index_list[exploit_start_index]
            for i in range(recording_start_index, len(self._first_datapacket_of_exploit_recording_index_list)):
                if self._first_datapacket_of_exploit_recording_index_list[i] > exploit_window_start:
                    exploit_window_end = self._first_datapacket_of_exploit_recording_index_list[i]
                    recording_start_index = i
                    break
            ax2.axvspan(exploit_window_start, exploit_window_end, color="lightcoral")
            exploit_start_index += 1
            if exploit_start_index == len(self._first_datapacket_after_exploit_index_list):
                done = True

        # cfp windows for exploit subplot
        for i, j in zip(self._first_datapacket_of_cfp_list_exploit, self._last_datapacket_of_cfp_list_exploit):
